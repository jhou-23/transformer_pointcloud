...
def discretize(point_set, bins, rasterize=False):
        batch, npoints, _ = point_set.shape
        bins -= 1
        # create nx3 matrix of offsets that move points to furthest x,y,zs to 0,0,0
        offset = (torch.Tensor([torch.min(point_set[:,0]), torch.min(point_set[:,1]), torch.min(point_set[:,2])]).repeat(npoints,1)).cuda()
        discretized = (point_set - offset).cuda()
        # scale matrix which finds largest in each direction and scales based of of bins
        scale = (float(bins) / torch.max(discretized)).cuda()
        if scale.isnan() or scale.isinf():
            scale = (torch.Tensor(bins)[0]).cuda()

        # discretized = discretized.mul(scale) # hadamard's product
        discretized *= scale
        discretized = torch.round(discretized).cuda() # round all elements

        if rasterize:
            point_list = []

            for i in range(len(point_set)):
                point_list.append((point_set[i, :], discretized[i, :]))
        ...
        return pc.cuda(), discretized.cuda()
