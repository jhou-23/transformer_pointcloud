...
#         data_augmentation=False)
    ...
    train_dataset = ModelNet40(root=config["dataset"]["path"], npoint=config["dataset"]["num_points"], split="train", normal_channel=False)
    test_dataset = ModelNet40(root=config["dataset"]["path"], npoint=config["dataset"]["num_points"], split="test", normal_channel=False)
else:
...
if config["optimizer"]["type"] == "Adam":
    if "eps" in config["optimizer"].keys():
        optimizer = optim.Adam(classifier.parameters(), lr=float(config["optimizer"]["lr"]), betas=config["optimizer"]["betas"], eps=float(config["optimizer"]["eps"]), weight_decay=float(config["optimizer"]["weight_decay"]))
    else:
        optimizer = optim.Adam(classifier.parameters(), lr=float(config["optimizer"]["lr"]), betas=config["optimizer"]["betas"], weight_decay=float(config["optimizer"]["weight_decay"]))
elif config["optimizer"]["type"] == "SGD":
    optimizer = optim.SGD(classifier.parameters(), lr=float(config["optimizer"]["lr"]), momentum=float(config["optimizer"]["momentum"]), weight_decay=float(config["optimizer"]["weight_decay"]))
...
for epoch in range(config["train"]["epochs"]):
    ...
    for i, data in enumerate(train_loader, 0):
        if config["dataset"]["type"] == 'modelnet40':
            points, target = data
            ...
            target = target[:, 0]
            if config["model"]["components"]["input_embedding"]["pos_enc"] == 'ape' and not config["model"]["components"]["input_embedding"]["neighbor"]:
                points, discrete_points = provider.discretize(points, config["dataset"]["bins"])
            else:
                discrete_points = points[:,:,0:3]
            points, target, discrete_points = points.cuda(), target.cuda(), discrete_points.cuda()
            target = target.long()
        else:
